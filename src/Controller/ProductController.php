...
//                    $this->request->getData('countryoforigin') . "%",
//                    "p_sale_price_money <=" => $this->request->getData("price")])
//                ->order(['p_name' => 'asc'])
//                ->contain(['Category']);
            ECHO 'HERE';
            $query = $product = $this-> Product -> find()->matching(
                'Category', function ($q) {
                return $q->where(['Category.name LIKE' => "%" .
                    $this->request->getData('category') . "%",
                    'p_country_of_origin LIKE' => "%" .
...
//$this->request->getData = $_POST
        ...
        {
            $query = $product = $this -> Product-> Category -> find();
            $query ->matching(
                'Category', function ($q) {
                return $q->where(['Category.c_name LIKE' => $this->request->getData("category")]);
